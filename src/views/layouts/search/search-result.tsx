...
import { CHANGE_URL_WITH_DATA_EVENT } from "hornet-js-core/src/event/hornet-event";
...
export var SELECTED_RESULT_CHANGE = new HornetEvent<SelectedResultChangeDetail>("SELECTED_RESULT_CHANGE");
...
export class SearchResult extends HornetComponent<SearchResultProps, any> {
    ...
    render(): JSX.Element {
        ...
        let text = this.props.item.all.replace(new RegExp(this.props.item.word, 'gi'), "<span class=\"search-word\">" + this.props.item.word + "</span>");
        ...
        let className = this.state.isActive ? "result-container result-container-active" : "result-container";
        return (
    ...
    openDoc() {
        ...
        this.fire(CHANGE_URL_WITH_DATA_EVENT.withData({ url: url, data: { word: this.props.item.word }, cb: null }));
    }
